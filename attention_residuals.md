...
The block attention applies a two-phase algorithm that (1) calculates the attention within blocks in parallel and (2) applies the attention recursively with running softmax. 

An important consideration is that each layers uses a learned pseudo-query; meaning it always uses the same query vector, independent of the input. 

Results show across-the-board improvement over baseline and mHC.
